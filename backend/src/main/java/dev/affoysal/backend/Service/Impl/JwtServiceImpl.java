...
import dev.affoysal.backend.Security.JwtConfiguration;
...
public class JwtServiceImpl extends JwtConfiguration implements JwtService {
        ...
        private final BiFunction<HttpServletRequest, String, Optional<String>> extractToken = (request,
                        cookieName) -> Optional
                                        .of(Arrays
                                                        .stream(request.getCookies() == null
        ...
        private final Supplier<JwtBuilder> builder = () -> Jwts.builder()
                        .header().type("JWT")
                        .and()
                        .audience().add(Constants.REV_DOJO_LLC)
                        .and()
        ...
        private final BiFunction<User, TokenType, String> buildToken = (user,
                        type) -> Objects.equals(type, TokenType.ACCESS) ? builder.get()
                                        ...
                                        : builder.get()
                                                        ...
                                                        .expiration(Date.from(
                                                                        Instant.now().plusSeconds(getExpiration())))
                                                        .compact();
        ...
        public Function<String, List<GrantedAuthority>> authorities = token -> AuthorityUtils
                        .commaSeparatedStringToAuthorityList(new StringJoiner(Constants.AUTHORITY_DELIMITER)
                                        ...
                                        .add(Constants.ROLE_PREFIX
                                                        + claimsFunction.apply(token).get(Constants.ROLE, String.class))
                                        .toString());

        @Override
        public String createToken(User user, Function<Token, String> tokenFunction) {
                var token = Token.builder().access(buildToken.apply(user, TokenType.ACCESS))
                                .refresh(buildToken.apply(user, TokenType.REFRESH)).build();
                ...
                return tokenFunction.apply(
                                TokenData.builder()
                                                .valid(Objects.equals(
                                                                userService.getUserByEmail(subject.apply(token))
                                                                                .getEmail(),
